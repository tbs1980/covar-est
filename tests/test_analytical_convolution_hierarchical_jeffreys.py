...
import emcee
from covariance_estimation.analytical_covolution import (log_posterior_hierarchical_jeffreys_prior,
                                                         posterior_hierarchical_jeffreys_prior)
...
n = 20
omega_true = 1e-2
omega_bar = sum_y2/float(n+1) - zeta
print('omega_bar = ', omega_bar)

p_0 = [(1+np.random.rand(ndim)) for i in range(nwalkers)]
sampler = emcee.EnsembleSampler(nwalkers, ndim, log_posterior_hierarchical_jeffreys_prior, args=[zeta, sum_y2, n])
...
nrm = integrate.quad(posterior_hierarchical_jeffreys_prior, a=omega_min, b=omega_max, args=(zeta, sum_y2, n))
omega = np.linspace(omega_min, omega_max, 200)
post = np.zeros(len(omega))
for i in range(len(omega)):
    post[i] = posterior_hierarchical_jeffreys_prior(omega[i], zeta, sum_y2, n)
